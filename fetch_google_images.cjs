...
async function fetchImage(url, filepath) {
...
async function run() {
  console.log('Buscando imagens REAIS via googlethis...');
  ...
  for (const item of products) {
    ...
    try {
      const images = await google.image(item.name + ' png branco', { safe: false });
